...
import {
  ...
  observeIdentityBoundIntegrityStream,
  ...
  type ReleaseCandidateSourceEntry,
  type ValidatedReleaseCandidateInput
} from "./release-candidate.js";
import type { ReleaseScanCoverage } from "./release-readiness.js";
import {
...
type NodeOperations = Readonly<{
  ...
  openFile(path: string, flags: number): ReturnType<typeof open>;
...
export type NodeReleaseCandidatePreflightDependencies = Readonly<{
  platform?: NodeJS.Platform;
...
type FixtureOrLocalPreflightInput = Readonly<{
  addonName: string;
  target: Extract<PreflightReleaseCandidateToolInput["target"], { kind: "fixture" | "local" }>;
}>;

const DEFAULT_OPERATIONS: NodeOperations = Object.freeze({
  lstat,
  realpath,
...
export function createNodeReleaseCandidateFilesystem(
  options: NodeReleaseCandidateFilesystemOptions = {}
  ...
  const lifecycle = createIdentityBoundCandidateLifecycle<NodeFileIdentity>({
    ...
    readAcceptedSourceFile: async (input, entry, maxBytes) => (
      await readAcceptedSourceFile(input, entry, maxBytes, operations)
    ),
    observeAcceptedSourceEntry: async (input, entry) => (
      await observeAcceptedSourceEntry(input, entry, operations)
    ),
    observeAcceptedSource: async (input, entry) => (
      await observeSourceIntegrity(input, entry, operations)
    ),
    observeCandidate: async (identity, expected) => (
      await observeCandidateIntegrity(identity, expected, operations)
    ),
    ...
    materializeCandidateEntry: async (identity, input, operation) => (
      await materializeCandidateEntry(identity, input, operation, operations)
    ),
    reconcileCandidateTree: async (identity, expected) => (
      await reconcileCandidateTree(identity, expected, operations)
    )
  });
  ...
  return Object.freeze({
    ...
    createCandidateRoot: async (input) => await operations.createTemporaryDirectory(
      join(input.tempParent, "dota-release-candidate-")
    ),
    candidateLifecycle: lifecycle
  });
}

export async function preflightNodeReleaseCandidate(
  input: PreflightReleaseCandidateToolInput,
  dependencies: NodeReleaseCandidatePreflightDependencies = {}
): Promise<ReleaseCandidateDetail> {
  if (input.target.kind === "remote") {
    return normalizeReleaseCandidateDetail(undefined);
  }
  const fixtureOrLocalInput: FixtureOrLocalPreflightInput = {
...
function projectLifecycleDetail(
  input: FixtureOrLocalPreflightInput,
  lifecycle: ReleaseCandidateLifecycleResult
...
function projectScanCoverage(
  coverage: ReleaseScanCoverage | undefined,
  addonName: string
): ReleaseScanCoverage | undefined {
  ...
  const category = (value: ReleaseScanCoverage["text"]) => ({
    count: value.count,
...
async function captureCandidateIdentity(path: string, operations: NodeOperations): Promise<NodeFileIdentity> {
  const stats = await operations.lstat(path);
...
async function identityMatches(identity: NodeFileIdentity, operations: NodeOperations): Promise<boolean> {
  try {
...
async function cleanupCandidate(
  identity: NodeFileIdentity,
  operations: NodeOperations
): Promise<CandidateLeaseCleanupResult> {
...
async function readAcceptedSourceFile(
  ...
  if (path === undefined || entry.kind !== "file") return { ok: false, code: "SOURCE_FILE_IDENTITY_CHANGED" };
  try {
    const handle = await operations.openFile(path, filesystemConstants.O_RDONLY | filesystemConstants.O_NOFOLLOW);
    try {
      const stats = await handle.stat();
      if (!stats.isFile()) return { ok: false, code: "SOURCE_FILE_IDENTITY_CHANGED" };
      const facts = {
        ok: true as const,
      ...
      if (!isReleaseTextPath(entry.path)) return { ...facts, state: "binary" };
      if (stats.size > maxBytes) return { ...facts, state: "oversized" };
...
async function observeAcceptedSourceEntry(
  input: ValidatedReleaseCandidateInput,
  entry: ReleaseCandidateSourceEntry,
  operations: NodeOperations
): Promise<AcceptedSourceObservationResult> {
  const path = sourcePath(input, entry);
  if (path === undefined) return { ok: false, code: "SOURCE_ENTRY_CHANGED" };
  try {
    const stats = await operations.lstat(path);
...
async function observeSourceIntegrity(
  input: ValidatedReleaseCandidateInput,
  entry: ReleaseCandidateSourceEntry,
  operations: NodeOperations
): Promise<unknown> {
  const path = sourcePath(input, entry);
  if (path === undefined || entry.kind !== "file") return { ok: false, code: "SOURCE_INTEGRITY_IDENTITY_CHANGED" };
  return await streamIntegrity(path, entry.root, entry.path, operations);
}

async function streamIntegrity(
  path: string,
  root: "game" | "content",
  identity: string,
  operations: NodeOperations
...
async function inspectCandidateRoot(
  identity: NodeFileIdentity,
  operations: NodeOperations
): Promise<CandidateRootInspectionResult> {
  if (!await identityMatches(identity, operations)) {
...
async function materializeCandidateEntry(
  identity: NodeFileIdentity,
  input: ValidatedReleaseCandidateInput,
  ...
  try {
    ...
    if (operation.kind === "directory") {
      ...
      if (source === undefined) return { ok: false, code: "SOURCE_ENTRY_CHANGED" };
      ...
      try {
        ...
        if (!sourceStats.isFile()) return { ok: false, code: "SOURCE_ENTRY_CHANGED" };
        const destinationHandle = await operations.openFile(
          destination,
...
async function reconcileCandidateTree(
  identity: NodeFileIdentity,
  ...
  try {
    ...
    const issues: NonNullable<Extract<CandidateTreeReconciliationResult, { ok: false }>["issues"]> = [];
    for (const entry of expected) {
      const actualKind = actualMap.get(entry.path);
      if (actualKind === undefined) issues.push({ code: "CANDIDATE_TREE_MISSING", path: entry.path });
      else if (actualKind !== entry.kind) issues.push({ code: "CANDIDATE_TREE_WRONG_KIND", path: entry.path, kind: actualKind });
    }
...
async function walkCandidate(
  identity: NodeFileIdentity,
  operations: NodeOperations
): Promise<CandidateExpectedEntry[]> {
  const output: CandidateExpectedEntry[] = [];
  const walk = async (directory: string): Promise<void> => {
    const names = [...await operations.readDirectory(directory)].sort(compareOrdinal);
    for (const name of names) {
      if (!safeName(name)) throw new Error("invalid candidate identity");
...
async function observeCandidateIntegrity(
  identity: NodeFileIdentity,
  expected: CandidateExpectedEntry[],
  operations: NodeOperations
): Promise<unknown> {
  ...
  const observations: FileIntegrityObservation[] = [];
  for (const entry of expected) {
    if (entry.kind !== "file" || !safeRelativeIdentity(entry.path)) continue;
    const path = resolve(identity.root, ...entry.path.split("/"));
    if (!isInside(path, identity.root)) return { ok: false, code: "CANDIDATE_INTEGRITY_IDENTITY_CHANGED" };
    ...
    if (observed === null || typeof observed !== "object" || Reflect.get(observed, "ok") !== true) return observed;
    observations.push(observed as FileIntegrityObservation);
  }
  return { ok: true, schemaVersion: "1.0", observations };
}

async function isRegularNonLink(path: string, operations: NodeOperations): Promise<boolean> {
  try {
    const stats = await operations.lstat(path);
...
function safeRelativeIdentity(value: string): boolean {
