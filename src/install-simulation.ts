...
export type InstallSimulationCleanup = {
  attempted: boolean;
  removed: boolean;
};
...
export type SimulateLocalInstallInput = {
  root?: string;
  tempParent?: string;
  cleanup?: boolean;
};
...
export async function simulateLocalInstall(input: SimulateLocalInstallInput = {}): Promise<InstallSimulationResult> {
  ...
  const blockers: InstallSimulationBlocker[] = [];
  ...
  const cleanup: InstallSimulationCleanup = {
...
async function createSimulationRoot(tempParent: string): Promise<string> {
...
async function checkConsumerContract(
...
async function readJson(path: string, relativePath: string, blockers: InstallSimulationBlocker[]): Promise<unknown | undefined> {
  try {
...
async function scanSimulationForSensitiveMaterial(simulationRoot: string, blockers: InstallSimulationBlocker[]): Promise<void> {
...
function sameEnvironment(left: Record<string, string | undefined>, right: Record<string, string | undefined>): boolean {
