...
type CleanupPayload = {
  ...
  dryRun?: boolean;
  matchedCount?: number;
  stoppedProcessIds?: number[] | number;
  processes?: CleanupProcess[] | CleanupProcess;
};

export type CleanupPlayableSmokeInput = {
  target: Target;
  addonName: string;
  dryRun?: boolean;
  executor?: (command: CommandEvidence) => Promise<CleanupOutput>;
};

export async function cleanupPlayableSmoke(input: CleanupPlayableSmokeInput): Promise<ToolResult> {
...
function validateCleanupInput(input: CleanupPlayableSmokeInput, operation: string): ToolResult | undefined {
...
function cleanupFromStdout(input: {
  ...
  try {
    const payload = JSON.parse(input.stdout || "{}") as CleanupPayload;
...
function cleanupProcessScript(addonName: string, dryRun: boolean): string {
...
function localCleanupCommand(script: string): CommandEvidence {
  return {
    command: `powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command "${script}"`
  };
}

function cleanupPaths(target: Target): Record<string, string> {
...
function cleanupWarnings(extra: string[] = []): string[] {
  return [
...
function normalizeProcesses(value: CleanupPayload["processes"]): CleanupProcess[] {
  if (!value) {
    return [];
  }
...
function normalizeNumbers(value: CleanupPayload["stoppedProcessIds"]): number[] {
  if (value === undefined) {
    return [];
  }
...
async function defaultExecutor(command: CommandEvidence): Promise<CleanupOutput> {
  try {
