...
export function buildRemoteExportedCandidateScript(input: RemoteExportScriptInput): string {
  ...
  const statements = [
    ...
    "    try {",
    `      $exportRoot = [IO.Path]::GetFullPath($ExportRootInput).TrimEnd('\\', '/'); $destination = [IO.Path]::GetFullPath($DestinationInput).TrimEnd('\\', '/'); $handoffPath = $destination + ${handoffSuffix}`,
    ...
    "      if ((Test-Path -LiteralPath $destination) -or (Test-Path -LiteralPath $handoffPath)) { Stop-ReleaseCandidate 'EXPORT_DESTINATION_EXISTS' 'export-state' }",
