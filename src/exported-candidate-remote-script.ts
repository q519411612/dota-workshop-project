...
export function buildRemoteCleanupExportedCandidateScript(input: RemoteCleanupScriptInput): string {
  ...
  return [
    ...
    "  $handoffPath = $destination + $HandoffSuffix; $handoffItem = Get-Item -LiteralPath $handoffPath -Force; if (-not ($handoffItem -is [IO.FileInfo]) -or ($handoffItem.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) { throw 'HANDOFF_MANIFEST_INVALID' }; $handoffIdentity = Get-WindowsPathIdentity $handoffPath; $handoff = [IO.File]::ReadAllText($handoffPath, [Text.UTF8Encoding]::new($false, $true)) | ConvertFrom-Json; Assert-StrictHandoff $handoff $exportRoot $destination",
    ...
    "  $result.authorized = $true; $result.manifest = $handoff; $result.cleanup.authorized = $true; if ($DryRun) { $result.ok = $true; $result.code = $null; $result.cleanup.status = 'verified' } else { $result.cleanup.attempted = $true; $candidateTombstone = Join-Path $exportRoot ('.dota-workshop-candidate-delete-' + [Guid]::NewGuid().ToString('N')); $handoffTombstone = Join-Path $exportRoot ('.dota-workshop-handoff-delete-' + [Guid]::NewGuid().ToString('N') + '.json'); try { Assert-NoReparseAncestry $destination; $boundaryIdentity = Get-WindowsPathIdentity $destination; if ($boundaryIdentity.volumeIdentity -cne $identity.volumeIdentity -or $boundaryIdentity.fileIdentity -cne $identity.fileIdentity) { throw 'CANDIDATE_IDENTITY_MISMATCH' }; [IO.Directory]::Move($destination, $candidateTombstone); $movedIdentity = Get-WindowsPathIdentity $candidateTombstone; if ($movedIdentity.volumeIdentity -cne $identity.volumeIdentity -or $movedIdentity.fileIdentity -cne $identity.fileIdentity) { throw 'CANDIDATE_IDENTITY_MISMATCH' }; $movedObserved = Get-ObservedCandidate $candidateTombstone; if ((ConvertTo-Json $movedObserved -Depth 20 -Compress) -cne (ConvertTo-Json $observed -Depth 20 -Compress)) { throw 'CANDIDATE_DIGEST_MISMATCH' }; Remove-Item -LiteralPath $candidateTombstone -Recurse -Force -ErrorAction Stop; $result.cleanup.candidateRemoved = $true } catch {}; $result.cleanup.candidateAbsent = $result.cleanup.candidateRemoved -and -not (Test-Path -LiteralPath $destination) -and -not (Test-Path -LiteralPath $candidateTombstone); if ($result.cleanup.candidateAbsent) { try { $currentHandoffIdentity = Get-WindowsPathIdentity $handoffPath; if ($currentHandoffIdentity.volumeIdentity -cne $handoffIdentity.volumeIdentity -or $currentHandoffIdentity.fileIdentity -cne $handoffIdentity.fileIdentity) { throw 'HANDOFF_IDENTITY_MISMATCH' }; [IO.File]::Move($handoffPath, $handoffTombstone); $movedHandoffIdentity = Get-WindowsPathIdentity $handoffTombstone; if ($movedHandoffIdentity.volumeIdentity -cne $handoffIdentity.volumeIdentity -or $movedHandoffIdentity.fileIdentity -cne $handoffIdentity.fileIdentity) { throw 'HANDOFF_IDENTITY_MISMATCH' }; Remove-Item -LiteralPath $handoffTombstone -Force -ErrorAction Stop; $result.cleanup.manifestRemoved = $true } catch {} }; $result.cleanup.manifestAbsent = $result.cleanup.manifestRemoved -and -not (Test-Path -LiteralPath $handoffPath) -and -not (Test-Path -LiteralPath $handoffTombstone); $result.ok = $result.cleanup.candidateRemoved -and $result.cleanup.candidateAbsent -and $result.cleanup.manifestRemoved -and $result.cleanup.manifestAbsent; $result.cleanup.status = $(if ($result.ok) { 'verified' } else { 'failed' }); $result.code = $(if ($result.ok) { $null } else { 'EXPORTED_CANDIDATE_CLEANUP_INCOMPLETE' }) }",
