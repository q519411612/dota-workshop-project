...
export function buildRemoteExportedCandidateScript(input: RemoteExportScriptInput): string {
  ...
  const statements = [
    ...
    "    $exportStaging = $null; $exportPromoted = $false; $temporaryHandoff = $null; $exportRoot = $null; $destination = $null; $handoffPath = $null; $handoff = $null",
    "    try {",
    ...
    "      [IO.Directory]::Move($exportStaging, $destination); $exportPromoted = $true; $result.exportState = [ordered]@{ schemaVersion = '1.0'; promotionState = 'promoted'; candidateState = 'present' }",
    ...
    "      $result.export = $handoff",
    "    } finally {",
    "      $temporaryHandoffRemoved = $false; if ($null -ne $temporaryHandoff -and (Test-Path -LiteralPath $temporaryHandoff)) { try { Remove-Item -LiteralPath $temporaryHandoff -Force -ErrorAction Stop; $temporaryHandoffRemoved = $true } catch {} }; $temporaryHandoffAbsent = $null -eq $temporaryHandoff -or -not (Test-Path -LiteralPath $temporaryHandoff)",
    "      $stagingRemoved = $false; if (-not $exportPromoted -and $null -ne $exportStaging -and (Test-Path -LiteralPath $exportStaging)) { try { Remove-Item -LiteralPath $exportStaging -Recurse -Force -ErrorAction Stop; $stagingRemoved = $true } catch {} }; $stagingAbsent = $exportPromoted -or $null -eq $exportStaging -or -not (Test-Path -LiteralPath $exportStaging)",
    "      if ($null -ne $exportRoot -and $null -ne $destination -and $null -ne $handoffPath) { $result.exportPaths = [ordered]@{ exportRoot = $exportRoot; destination = $destination; handoffManifest = $handoffPath } }",
    "      if ($null -eq $result.exportState) { $candidateState = $(if ($exportPromoted) { 'present' } elseif ($null -ne $destination -and (Test-Path -LiteralPath $destination)) { 'unknown' } else { 'absent' }); $result.exportState = [ordered]@{ schemaVersion = '1.0'; promotionState = $(if ($exportPromoted) { 'promoted' } else { 'not-started' }); candidateState = $candidateState } }",
    "      $exportCode = $(if ($result.blockers.Count -gt 0) { $result.blockers[0].code } elseif ($null -ne $result.export) { $null } else { 'REMOTE_EXPORT_INTERNAL_FAILURE' }); $result.exportCleanup = [ordered]@{ schemaVersion = '1.0'; mode = 'export-failure'; authorized = ($null -ne $result.export); attempted = $false; candidateRemoved = $false; candidateAbsent = ($result.exportState.candidateState -eq 'absent'); manifestRemoved = $false; manifestAbsent = ($null -eq $handoffPath -or -not (Test-Path -LiteralPath $handoffPath)); candidateState = $result.exportState.candidateState; manifestState = $(if ($null -ne $handoffPath -and (Test-Path -LiteralPath $handoffPath)) { 'present' } else { 'absent' }); stagingRemoved = $stagingRemoved; stagingAbsent = $stagingAbsent; temporaryHandoffRemoved = $temporaryHandoffRemoved; temporaryHandoffAbsent = $temporaryHandoffAbsent; promotionState = $result.exportState.promotionState; status = $(if ($null -eq $exportCode -and $stagingAbsent -and $temporaryHandoffAbsent) { 'verified' } else { 'failed' }) }; if ($null -ne $exportCode) { $result.exportCleanup.code = $exportCode }",
    "    }"
  ];
...
export function buildRemoteCleanupExportedCandidateScript(input: RemoteCleanupScriptInput): string {
  ...
  return [
    ...
    "  $result.authorized = $true; $result.manifest = $handoff; $result.cleanup.authorized = $true; $result.cleanup.candidateState = 'present'; $result.cleanup.manifestState = 'present'; if ($DryRun) { $result.ok = $true; $result.code = $null; $result.cleanup.Remove('code') | Out-Null; $result.cleanup.status = 'verified' } else { Invoke-ExportedCandidateCleanup $result $exportRoot $destination $handoffPath $handoffLease $handoffIdentity $identity $observed }",
    "} catch { $result.code = $(if ($_.Exception.Message -match '^[A-Z0-9_]+$') { $_.Exception.Message } else { 'CLEANUP_AUTHORIZATION_FAILED' }); $result.cleanup.code = $result.code }",
...
export function buildRemoteHandoffLeaseProbeScript(handoffPath: string, replacementPath: string): string {
