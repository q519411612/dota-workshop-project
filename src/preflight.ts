...
async function collectReleaseReadinessObservations(paths: Record<string, string>): Promise<ReleaseReadinessInput> {
  const requiredPaths = [
  ...
  const metadata: ReleaseReadinessInput["metadata"] = (await pathExists(paths.addonInfo))
