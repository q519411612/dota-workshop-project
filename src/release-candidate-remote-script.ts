...
export function buildRemoteReleaseCandidateScript(input: RemoteReleaseCandidateScriptInput): string {
  ...
  return [
    ...
    "function Test-SensitiveIdentity([string]$Relative) { $patterns = @('-----BEGIN [A-Z ]*PRIVATE KEY-----', 'gh[pousr]_[A-Za-z0-9_]{20,}', '\\bsteam_(?:password|token|secret|apikey|api_key)\\b', '(?:\\b|_)(?:password|passwd|pwd)\\b\\s*[:=]', '\\b(?:token|api[_-]?key|secret)\\b\\s*[:=]', '\\b(?:remote_|windows_)?(?:host|username)\\b\\s*[:=].*\\b(?:password|token|secret|key)\\b'); foreach ($segment in $Relative.Split('/')) { foreach ($pattern in $patterns) { if ([regex]::IsMatch($segment, $pattern, [Text.RegularExpressions.RegexOptions]::IgnoreCase)) { return $true } } }; return $false }",
    "function Assert-SafeSourceRoot([string]$DotaRoot, [string]$Root) { if (-not (Test-ContainedPath $DotaRoot $Root)) { throw 'SOURCE_ENTRY_OUTSIDE_ROOT' }; $dotaFull = [IO.Path]::GetFullPath($DotaRoot).TrimEnd('\\', '/'); $current = [IO.Path]::GetFullPath($Root).TrimEnd('\\', '/'); while ($true) { $item = Get-Item -LiteralPath $current -Force; if (-not $item.PSIsContainer -or ($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) { throw 'SOURCE_ENTRY_UNSAFE' }; if ($current.Equals($dotaFull, [StringComparison]::OrdinalIgnoreCase)) { break }; $parent = [IO.Path]::GetDirectoryName($current); if ([string]::IsNullOrEmpty($parent) -or ($parent -ne $dotaFull -and -not (Test-ContainedPath $dotaFull $parent))) { throw 'SOURCE_ENTRY_OUTSIDE_ROOT' }; $current = $parent.TrimEnd('\\', '/') } }",
    "function Assert-SafeSourceTree([string]$Root) { $seen = [Collections.Generic.HashSet[string]]::new([StringComparer]::OrdinalIgnoreCase); foreach ($entry in @(Get-ChildItem -LiteralPath $Root -Recurse -Force)) { if (($entry.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) { throw 'SOURCE_ENTRY_UNSAFE' }; if (-not ($entry.PSIsContainer -or $entry -is [IO.FileInfo])) { throw 'SOURCE_ENTRY_UNSAFE' }; $relative = Get-SafeRelativePath $Root $entry.FullName; if (Test-SensitiveIdentity $relative) { Add-Blocker 'SOURCE_IDENTITY_SENSITIVE' 'source-identity'; throw 'SOURCE_IDENTITY_SENSITIVE' }; if (-not $seen.Add($relative)) { throw 'SOURCE_IDENTITY_COLLISION' } } }",
    "function Get-SourceInventory([string]$RootKind, [string]$Root) { $files = @(); $directories = @(); foreach ($entry in @(Get-ChildItem -LiteralPath $Root -Recurse -Force)) { $relative = Get-SafeRelativePath $Root $entry.FullName; $identity = Get-Identity $RootKind $relative; if ($entry.PSIsContainer) { $directories += [ordered]@{ root = $RootKind; relative = $relative; identity = $identity; sourceRoot = $Root; source = $entry.FullName } } elseif ($entry -is [IO.FileInfo]) { $files += [ordered]@{ root = $RootKind; relative = $relative; identity = $identity; sourceRoot = $Root; source = $entry.FullName; bytes = [long]$entry.Length } } else { throw 'SOURCE_ENTRY_UNSAFE' } }; return [ordered]@{ files = @($files); directories = @($directories) } }",
    "function Assert-SafeSourceFile([object]$File) { $item = Get-Item -LiteralPath $File.source -Force; if (-not ($item -is [IO.FileInfo]) -or ($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) { throw 'SOURCE_ENTRY_CHANGED' }; if (-not (Test-ContainedPath $File.sourceRoot $item.FullName)) { throw 'SOURCE_ENTRY_OUTSIDE_ROOT' }; $relative = Get-SafeRelativePath $File.sourceRoot $item.FullName; if (-not $relative.Equals($File.relative, [StringComparison]::Ordinal) -or -not (Get-Identity $File.root $relative).Equals($File.identity, [StringComparison]::Ordinal)) { throw 'SOURCE_FILE_IDENTITY_CHANGED' } }",
    ...
    "function Assert-CandidateRoot([string]$CandidateRoot, [string]$Identity) { $parent = [IO.Path]::GetTempPath(); $item = Get-Item -LiteralPath $CandidateRoot -Force; if (-not $item.PSIsContainer -or ($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0 -or -not (Test-ContainedPath $parent $CandidateRoot) -or -not ([IO.Path]::GetFileName($CandidateRoot)).Equals($Identity, [StringComparison]::Ordinal) -or @(Get-ChildItem -LiteralPath $CandidateRoot -Force).Count -ne 0) { throw 'CANDIDATE_ROOT_NOT_ISOLATED' } }",
    ...
    "function Assert-CandidateTopology([object[]]$Directories, [string]$CandidateRoot) { foreach ($directory in $Directories) { $candidate = Join-Path $CandidateRoot $directory.identity; if (-not (Test-Path -LiteralPath $candidate -PathType Container)) { throw 'CANDIDATE_TREE_MISSING' } }; $expectedSet = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal); foreach ($identity in @('game', 'game/dota_addons', 'game/dota_addons/' + $AddonName, 'content', 'content/dota_addons', 'content/dota_addons/' + $AddonName) + $Directories.identity + $script:allFiles.identity) { $expectedSet.Add($identity) | Out-Null }; $expected = @($expectedSet); [Array]::Sort($expected, [StringComparer]::Ordinal); $observed = @(); foreach ($item in @(Get-ChildItem -LiteralPath $CandidateRoot -Recurse -Force)) { $observed += (Get-SafeRelativePath $CandidateRoot $item.FullName).Replace('\\', '/') }; [Array]::Sort($observed, [StringComparer]::Ordinal); if ((ConvertTo-Json -InputObject $expected -Compress) -ne (ConvertTo-Json -InputObject $observed -Compress)) { throw 'CANDIDATE_TREE_UNEXPECTED' } }",
    "function Get-CanonicalManifestDigest([object[]]$Entries) { $rows = @(); foreach ($entry in $Entries) { $rows += ,@($entry.schemaVersion, $entry.root, $entry.path, [long]$entry.bytes, $entry.sha256) }; return Get-TextSha256 (ConvertTo-Json -InputObject $rows -Depth 4 -Compress) }",
    "if ((Get-TextSha256 $CanonicalVectorJson) -ne $CanonicalVectorSha256 -or (Get-CanonicalManifestDigest $CanonicalVectorEntries) -ne $CanonicalVectorSha256) { throw 'CANDIDATE_MANIFEST_PROJECTION_FAILED' }",
    ...
    "    $created = New-CandidateRoot; $candidateRoot = $created.path; $candidateIdentity = $created.identity; Assert-CandidateRoot $candidateRoot $candidateIdentity",
