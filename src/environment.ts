...
export type ValidateInstallRootInput = {
  target: Target;
  dotaRoot: string;
};

export type DiscoverEnvironmentInput = {
  target: Target;
  platform?: NodeJS.Platform;
  environment?: NodeJS.ProcessEnv;
};

export async function validateInstallRoot(input: ValidateInstallRootInput): Promise<ToolResult> {
...
export async function discoverEnvironment(input: DiscoverEnvironmentInput): Promise<ToolResult> {
...
async function discoverCandidateRoots(environment: NodeJS.ProcessEnv): Promise<string[]> {
...
async function readSteamLibraryRoots(steamRoot: string): Promise<string[]> {
  try {
...
function installPaths(dotaRoot: string): Record<string, string> {
  return {
    dotaRoot,
...
async function checkRequiredPath(
  path: string,
  presentText: string,
  missingText: string,
  evidence: string[],
