...
export type ReleaseCandidateScanResult = {
  ok: boolean;
  evidence: string[];
  warnings: string[];
  blockers: ReleaseCandidateBlocker[];
  paths: Record<string, string>;
};

export type ReleaseCandidateVerificationResult = ReleaseCandidateScanResult & {
  commands: ReleaseCandidateCommandResult[];
};

export type ReleaseCandidateVerificationInput = {
  root?: string;
  commandRunner?: (command: string, options: { cwd: string }) => Promise<ReleaseCandidateCommandResult>;
};

export type ReleaseCandidateScanInput = {
  root?: string;
  maxFileBytes?: number;
};
...
type ScanRule = {
  label: string;
  code: "RC_FORBIDDEN_CONTENT" | "RC_FORBIDDEN_PUBLISHING_BOUNDARY";
  pattern: RegExp;
};
...
const boundaryToken = (...parts: string[]) => parts.join("");

const BOUNDARY_RULES: ScanRule[] = [
  {
    label: "workshop upload automation",
    code: "RC_FORBIDDEN_PUBLISHING_BOUNDARY",
    pattern: new RegExp(
      `\\b(?:${boundaryToken("workshop", "_", "build", "_", "item")}|${boundaryToken("workshop", "_", "upload")}|${boundaryToken("ugc", "_", "publish")}|${boundaryToken("Publish", "Workshop", "File")})\\b`,
      "i"
    )
  },
  {
  ...
  },
  {
    ...
    code: "RC_FORBIDDEN_PUBLISHING_BOUNDARY",
    pattern: new RegExp(
      `\\b(?:${boundaryToken("steam", "_", "guard", "_", "code")}|${boundaryToken("steamguard", "_", "code")}|${boundaryToken("steam", "_", "guard", "_", "automation")}|${boundaryToken("steamguard", "_", "automation")}|${boundaryToken("submit", "_", "steam", "_", "guard")})\\b`,
      "i"
    )
  },
  {
    ...
    code: "RC_FORBIDDEN_PUBLISHING_BOUNDARY",
    pattern: new RegExp(
      `\\b(?:${boundaryToken("encrypt", "_", "content")}|${boundaryToken("content", "_", "encryption")}|${boundaryToken("vpk", "_", "encrypt")}|${boundaryToken("dota", "_", "pak", "_", "encrypt")})\\b`,
      "i"
    )
  },
  {
    ...
    code: "RC_FORBIDDEN_PUBLISHING_BOUNDARY",
    pattern: new RegExp(
      `\\b(?:${boundaryToken("package", "_", "signing")}|${boundaryToken("sign", "_", "package")}|${boundaryToken("sign", "tool")}|${boundaryToken("code", "sign")})\\b`,
      "i"
    )
  },
  {
    ...
    code: "RC_FORBIDDEN_PUBLISHING_BOUNDARY",
    pattern: new RegExp(
      `\\b(?:${boundaryToken("publish", "_", "state")}|${boundaryToken("workshop", "_", "update")}|${boundaryToken("published", "file", "id")}|${boundaryToken("mutate", "_", "workshop")})\\b`,
      "i"
    )
  }
];
...
export async function verifyReleaseCandidate(
  input: ReleaseCandidateVerificationInput = {}
): Promise<ReleaseCandidateVerificationResult> {
  const root = input.root ?? process.cwd();
  const commandRunner = input.commandRunner ?? runShellCommand;
  const evidence: string[] = [];
  const warnings: string[] = [];
  const blockers: ReleaseCandidateBlocker[] = [];
  const commands: ReleaseCandidateCommandResult[] = [];
...
export async function scanReleaseCandidateFiles(input: ReleaseCandidateScanInput = {}): Promise<ReleaseCandidateScanResult> {
  const root = input.root ?? process.cwd();
...
async function* walkOwnedFiles(root: string, dir: string): AsyncGenerator<string> {
