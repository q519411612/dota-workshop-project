...
export type PluginVerificationIssue = {
  code: string;
  message: string;
  file: string;
  expected?: string[];
...
export type PluginVerificationResult = {
  ...
  evidence: string[];
  warnings: string[];
  blockers: PluginVerificationIssue[];
  paths: Record<string, string>;
};

export type VerifyPluginPackageInput = {
  root?: string;
};
...
export async function verifyPluginPackage(input: VerifyPluginPackageInput = {}): Promise<PluginVerificationResult> {
  const root = input.root ?? process.cwd();
  const evidence: string[] = [];
  const warnings: string[] = [];
  const blockers: PluginVerificationIssue[] = [];
  const paths = {
    root,
...
async function readJsonFile(path: string, label: string, blockers: PluginVerificationIssue[]): Promise<unknown | undefined> {
...
async function readTextFile(path: string, label: string, blockers: PluginVerificationIssue[]): Promise<string | undefined> {
  try {
...
function checkMcpConfig(config: unknown, file: string, blockers: PluginVerificationIssue[], evidence: string[]) {
...
function checkPackageJson(packageJson: unknown, file: string, blockers: PluginVerificationIssue[], evidence: string[]) {
...
function checkToolList(
  ...
  const extra = documentedTools.filter((tool) => !expected.includes(tool as typeof toolNames[number]));
...
function extractMarkdownToolList(content: string, heading: string): string[] {
  const section = extractSection(content, heading);
  return extractBacktickedBullets(section);
}

function extractSkillToolList(content: string): string[] {
...
function extractBacktickedBullets(content: string): string[] {
