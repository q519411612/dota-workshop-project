...
import type { CommandEvidence, RemoteTarget, Target, ToolResult } from "./types.js";
...
export type PrepareCustomMapCommandOutput = {
  exitCode: number;
  stdout: string;
  stderr: string;
};

export type PrepareCustomMapInput = {
  target: Target;
  addonName: string;
  mapName: string;
  ...
  replace?: boolean;
  executor?: (command: CommandEvidence) => Promise<PrepareCustomMapCommandOutput>;
};

type RemotePreparePayload = {
  ok: boolean;
  evidence?: string[];
  warnings?: string[];
...
export async function prepareCustomMap(input: PrepareCustomMapInput): Promise<ToolResult> {
  ...
  if (input.target.kind === "remote") {
    return prepareRemoteCustomMap(input as PrepareCustomMapInput & { target: RemoteTarget });
  }

  return prepareLocalCustomMap(input);
}

async function prepareLocalCustomMap(input: PrepareCustomMapInput): Promise<ToolResult> {
  const operation = "prepare_custom_map";
...
async function prepareRemoteCustomMap(
  input: PrepareCustomMapInput & { target: RemoteTarget }
): Promise<ToolResult> {
  const operation = "prepare_custom_map";
  ...
  let payload: RemotePreparePayload;
  try {
    payload = JSON.parse(stdout) as RemotePreparePayload;
  } catch {
    return createFailureResult({
      target: input.target,
      operation,
...
function validatePrepareInput(input: PrepareCustomMapInput, operation: string): ToolResult | undefined {
...
async function inspectSpawnMarkers(contentMap: string): Promise<{ ok: boolean; evidence: string[] }> {
...
async function executeLocalCompile(
  input: PrepareCustomMapInput,
  command: CommandEvidence
): Promise<{ ok: true; command: CommandEvidence } | { ok: false; result: ToolResult }> {
...
function mapPaths(root: string, input: PrepareCustomMapInput): Record<string, string> {
  const templateAddonName = input.templateAddonName ?? "addon_template";
  const templateMapName = input.templateMapName ?? "template_map";
  return {
    dotaRoot: root,
...
function compileCommand(paths: Record<string, string>): CommandEvidence {
  return {
...
function remotePrepareCustomMapScript(input: PrepareCustomMapInput & { target: RemoteTarget }): string {
...
function compileLogs(command: CommandEvidence) {
  return [
...
function compilePayloadLogs(payload: RemotePreparePayload) {
  if (!payload.compile) {
    return [];
  }
...
async function defaultExecutor(command: CommandEvidence): Promise<PrepareCustomMapCommandOutput> {
  try {
