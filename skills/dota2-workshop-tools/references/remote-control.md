...
Retained export and cleanup use `export_release_candidate` and `cleanup_exported_candidate`. Both run target-native scripts; addon and candidate bytes remain on Windows. The target strictly parses a parent-closed handoff topology and exhaustively reconciles the candidate before mutation. Windows handoff bytes are read once through a no-follow identity-bound Win32 handle held through candidate mutation; failure to establish that lease stops cleanup. The MCP host accepts only closed framed JSON and recomputes the full preflight and export invariants. Validated partial failures retain manifest, ownership, canonical paths, explicit tombstone paths, and object states. Execute uncertainty reports both candidate and handoff state as unknown and must not be retried automatically.
