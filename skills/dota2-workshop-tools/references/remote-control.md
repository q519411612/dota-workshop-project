...
Retained export and cleanup use `export_release_candidate` and `cleanup_exported_candidate`. Both run target-native scripts; addon and candidate bytes remain on Windows. The target strictly parses the handoff and exhaustively reconciles file-and-directory topology before mutation. The MCP host accepts only closed framed JSON, recomputes result invariants, redacts target details, and treats transport uncertainty as final. Cleanup dry-run and execute are distinct operations over the same exact authorization evidence; execute uncertainty reports both candidate and handoff state as unknown and must not be retried automatically.
