...
Before deletion, call `cleanup_exported_candidate` with `dryRun: true` and the exact path, ownership identifier, manifest version, and combined SHA-256 returned by export. Execute only after dry-run authorization succeeds. Export uses atomic no-replace promotion and publication. Cleanup must not delete through a revalidated pathname; when identity-bound deletion is unavailable, expect an explicit preserved tombstone result with the manifest, ownership, canonical paths, and exact object states. Remote execute uncertainty means both states are unknown. Never widen the path, repair a mismatch, or retry into mixed state.
