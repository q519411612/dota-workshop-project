...
Run `cleanup_exported_candidate` with `dryRun: true`. Proceed to execute mode only when the result says authorization passed. Windows authorization requires one no-follow handoff handle that binds the bytes read to the captured file identity and remains leased through candidate mutation; lease failure stops before mutation. Execute mode never treats pathname revalidation as identity-bound deletion. If the target cannot recursively delete the held identity, it preserves the no-replace candidate tombstone, keeps the handoff, and returns the validated manifest, ownership, canonical paths, tombstone path, and exact object states. Inspect that state manually; do not retry with wider paths or altered evidence. Remote transport uncertainty still means both object states are unknown.
