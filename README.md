...
Use `cleanup_exported_candidate` with `dryRun: true` first. Cleanup is authorized only when the canonical export root and destination, candidate identity, handle-bound non-reparse handoff bytes, ownership identifier, manifest version, complete parent-closed topology, and recomputed combined SHA-256 all match. Windows opens the handoff once through a no-follow Win32 handle and keeps that lease through candidate authorization and mutation; inability to establish the lease fails before mutation. Execute mode moves each verified object to an operation-owned same-parent tombstone, revalidates its identity, removes the candidate first, and removes the handoff only after candidate absence is proven. A failed removal restores an intact identity-matched object when possible; otherwise the result preserves the validated manifest, ownership, destination, and explicit tombstone or unknown state. Transport uncertainty during execute means both object states are unknown; do not retry automatically.
