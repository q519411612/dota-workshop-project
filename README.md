...
Use `cleanup_exported_candidate` with `dryRun: true` first. Cleanup is authorized only when the canonical export root and destination, candidate identity, non-link external handoff identity, ownership identifier, manifest version, complete topology, and recomputed combined SHA-256 all match. Execute mode moves each verified object to an operation-owned same-parent tombstone, revalidates its identity, removes the candidate first, and removes the handoff only after candidate absence is proven. Any mismatch, reparse point, unknown state, or partial removal is an explicit failure without repair or broad retry. Transport uncertainty during execute means both object states are unknown; do not retry automatically.
