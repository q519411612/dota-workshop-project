...
import { buildRemoteCleanupExportedCandidateScript, buildRemoteExportedCandidateScript } from "./exported-candidate-remote-script.js";
export async function executeRemoteExport(input) {
    if (input.target.kind !== "remote")
        return { transport: "ssh", outcome: "configuration-failed", code: "REMOTE_TARGET_REQUIRED" };
    const target = input.target;
    if (!target.dotaRoot)
        return { transport: target.transport, outcome: "configuration-failed", code: "REMOTE_DOTA_ROOT_REQUIRED" };
...
export async function executeRemoteExportCleanup(input) {
    if (input.target.kind !== "remote")
        return { transport: "ssh", outcome: "configuration-failed", code: "REMOTE_TARGET_REQUIRED" };
...
async function executeBuiltScript(target, build, executor) {
