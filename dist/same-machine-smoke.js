...
export function createHarnessReadySameMachineSmokeArtifact() {
    return {
        schemaVersion: "1.0",
...
export function verifySameMachineSmokeEvidence(artifact) {
...
function resolveRuntimeEvidence(status, logs) {
    const hasRuntimeMarker = logs.some((log) => log.lines.some((line) => line.includes("[DOTA_WORKSHOP_MCP]") && line.includes("win condition reached")));
    if (status === "runtime_passed") {
        return hasRuntimeMarker ? "passed" : "missing";
    }
    return "pending";
}
function findSensitiveStrings(value, field = "$") {
...
function blocker(code, field, category) {
