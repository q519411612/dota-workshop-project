...
export async function atomicMoveNoReplace(source, destination, platform = process.platform) {
    ...
    try {
        ...
        if (movement.exitCode === 17)
            throw new Error("ATOMIC_NO_REPLACE_DESTINATION_EXISTS");
        if (movement.exitCode !== 0 || movement.stderr.length !== 0)
            throw new Error("ATOMIC_NO_REPLACE_FAILED");
    }
    finally {
...
function encodedPowerShell(value) {
