...
export async function discoverRemoteEnvironment(input) {
...
export async function launchRemoteTools(input) {
...
export async function inspectRemoteAddon(input) {
...
export async function validateRemoteAddon(input) {
...
function buildRemoteCommand(target, command) {
...
function validateRemoteAddonInput(input, operation) {
    const nameValidation = validateAddonName(input.addonName);
    if (!nameValidation.ok) {
        return createFailureResult({
            target: input.target,
...
function remoteReadLogsScript(logPaths) {
    ...
    return `$ProgressPreference = 'SilentlyContinue'; $logs = @(); foreach ($path in @(${paths})) { if (Test-Path -LiteralPath $path) { $lines = @(Get-Content -LiteralPath $path -Tail ${REMOTE_LOG_TAIL_LINES} | ForEach-Object { [string]$_ }); $logs += @{ source = $path; lines = $lines } } }; $logs | ConvertTo-Json -Compress`;
}
